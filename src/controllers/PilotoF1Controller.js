...
class PilotoF1Controller {
    ...
    getPilotos = async (req, res) => {
        try {
            const piloto = await pilotoF1Service.getAllpiloto()
            res.json(piloto)
        } catch (error) {
            console.error(error)
    ...
    getPilotosFromApi = async (req, res) => {
        try {
            const piloto = await f1Service.getPiloto()
            res.json(piloto)
        } catch (error) {
            console.error(error)
            res.status(500).json({ error: 'Error al obtener pilotos' })
    ...
    syncPilotos = async (req, res) => {
        try {
            const pilotoFromApi = await f1Service.getPiloto()

            const result = await pilotoF1Service.syncPiloto(pilotoFromApi)
